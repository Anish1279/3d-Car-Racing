...
import { VEHICLE_CONFIG, VEHICLE_LOCAL_FORWARD } from '@/game/config/constants'
...
const isPerspectiveCamera = (camera: unknown): camera is PerspectiveCamera =>
...
export function Cameras({ chassisRef }: CamerasProps) {
  ...
  useFrame((state, delta) => {
    ...
    if (cameraMode === 'DEFAULT') {
      ...
      if (isPerspectiveCamera(camera)) {
        camera.fov = lerp(camera.fov, cfg.fov, 1 - Math.exp(-3 * dt))
        camera.updateProjectionMatrix()
      }
