...
function MinimapTexture({ buffer }: { buffer: WebGLRenderTarget }): JSX.Element {
  ...
  useFrame(() => {
    if (!camera.current || rendered.current) return
...
export function Minimap({ size = 180 }): JSX.Element {
  ...
  useFrame(() => {
    ...
    if (chassisBody.current && player.current) {
      ...
      player.current.position.set(
        screenPosition.x + (v.x / levelDimensions.x) * size,
        screenPosition.y - (v.z / levelDimensions.z) * size,
        0
      )
    ...
    <>
      {createPortal(
        <>
          ...
          <sprite ref={player} position={screenPosition} scale={[size / 18, size / 18, 1]}>
            <spriteMaterial color="#00d4ff" alphaMap={cursor} />
          </sprite>
        </>,
        virtualScene,
      )}
